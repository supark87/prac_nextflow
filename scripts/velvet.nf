...
params.outdir="result_tset_velvet"
...
process bbduk{
    ...
    output:
    tuple val(sample_id), file("${sample_id}_clean_{1,2}.fastq") into velvet_input
...
process run_prodigal{
    ...
    input:
    set val(sample_id), file(assembly) from velvet_output_for_prod
